...
Assert-True ($ebom.Count -eq 435) "Expected 435 current EBOM rows after ECO-011A2; found $($ebom.Count)."
Assert-True ($avl.Count -eq 435) "Expected 435 current AVL rows after ECO-011A2; found $($avl.Count)."
...
Assert-True ($outside.Count -eq 299) "Expected 299 out-of-scope rows after Sheet 05 decomposition; found $($outside.Count)."
