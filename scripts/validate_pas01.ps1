...
Assert-True ($rows.Count -eq 85) "Expected 85 passive rows; found $($rows.Count)."
...
Assert-True (@($rows|Where-Object Status -eq 'BLOCKED').Count -eq 18) 'Blocked count mismatch.'
...
foreach($path in $changed){Assert-True ($path -notmatch '\.kicad_sch$|\.kicad_pcb$|docs/adr/|docs/icd/') "Prohibited PAS-01 change: $path"}
