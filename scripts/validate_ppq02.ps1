...
Assert-True ([regex]::Matches($reg,'READY FOR PPC-01 \|').Count -eq 18) 'PPC route mismatch.'
Assert-True ([regex]::Matches($reg,'READY FOR PAS-01 \|').Count -eq 85) 'PAS route mismatch.'
Assert-True ([regex]::Matches($reg,'READY FOR PACS-01 \|').Count -eq 20) 'PACS route mismatch.'
Assert-True ([regex]::Matches($reg,'READY FOR JCS-01 \|').Count -eq 1) 'JCS route mismatch.'
foreach($token in @('0.980','1.246','26.4','60.1','0.635','0.959','11.55 J','500 A²s','162.82–222.35 mA')){Assert-True $all.Contains($token) "Missing calculation token $token."}
