...
Rev A combines a protected 9–21 V DC controller input, 5 V and 3.3 V logic rails, an ESP32-family module, wireless communications, local controls and a monochrome graphical OLED interface, environmental temperature/humidity/pressure sensing, battery monitoring, two external motor-driver logic interfaces, isolated relay contacts, and documented expansion provisions. ESP32-S3-WROOM-1 is the preferred processor module family, while its exact ordering variant and pin allocation remain open. The 2.42-inch SSD1309 OLED and BME280 remain reference implementations only.
