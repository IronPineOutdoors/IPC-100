...
- Relay coil actuation is main-only and subject to the common hardware master inhibit. Exact drive, isolation-boundary, transient-containment, and diagnostic implementations remain `TBD`.
...
- OUT-006 permits implementation alternatives only if the four-signal logical contract and common hardware master inhibit are preserved. The safe stop is disabled/coast; active braking is not a Rev A baseline feature. Reversal must pass through all commands inactive and all enables disabled; numeric timing remains `TBD`.
...
- The platform defines three main-only RGB logical channels whose hardware-safe state is inactive/off, including USB-only service. LED package, topology, current, brightness, optical diffuser, placement, and product color meanings remain unresolved or product-level decisions.
- `BUZZER_OUT` is main-only and hardware-default-silent, including USB-only service. It may use an active buzzer, passive transducer, external sounder, or another approved device; final topology is not selected.
- `OLED_RESET` defaults logically asserted or electrically non-driving while `OLED_VCC` is invalid or off. It releases only after the approved display power sequence and shall not backfeed an unpowered display.
