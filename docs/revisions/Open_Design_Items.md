...
| ODI-OUT-002 | Motor-driver compatibility | Approve logic levels, thresholds, source/sink current, impedance, grounding, cable, protection, and independent-power behavior | Reference signaling is not universal compatibility | J2, J3, external drivers | Before schematic release | Logical/safe contract selected; electrical contract TBD |
| ODI-OUT-003 | Master-inhibit implementation | Implement common hardware override of all motor signals and relay authorization from STOP/main-valid/reset conditions | Firmware cannot be the sole safe-state mechanism | GPIO map, inputs, power, safety | Before schematic release | Behavior/coverage selected; circuit/fault analysis TBD |
