...
| ODI-SCH-017 | Sheet 06 watchdog-service interface | Allocate and route a supervised firmware watchdog-service signal from Sheet 03 to the Sheet 06 independent watchdog; define polarity, timing, reset default, and test-access disposition | The frozen Sheet 06 boundary requires `WATCHDOG_VALID` in `ACTUATOR_PERMIT` but exposes no watchdog-service input and ADR-040 allocates no GPIO for one | ADR-040; ADR-042; External Safety ICD; Package 07 entry-gate record | Before Sheet 06 capture | Open — blocking; architecture/interface resolution required |
