...
> **ECO-008 status:** The TPS2553 legal current-limit window is mathematically empty, so ECO-008 stopped without a design change. [QER-02 Branch Peak and Protection Ceiling Reconciliation](hardware/kicad/notes/ECO-008_TPS2553_Branch_Limit_Compliance.md) is required before CSR-01A-R4.
...
Rev A uses the ESP32 processor family with Wi-Fi, Bluetooth, and ESP-NOW. ESP32-S3-WROOM-1-N8 is selected for preliminary Rev A capture. ADR-040 allocates every brought-out GPIO without conflict, preserves native USB and UART0 recovery, assigns four regulated-rail request outputs, moves five low-risk UI functions behind the controlled I²C boundary, and reserves a two-pin pool for one future communications option. ADR-041 keeps qualified-main detection and response in hardware and removes unneeded `MAIN_POWER_GOOD` firmware visibility, preserving every GPIO assignment and USB-only recovery. IPC-100 accepts 9–21 V DC during normal operation, creates 5 V and 3.3 V logic rails, and provides a USB-C service interface, battery monitoring, protected inputs, a local monochrome graphical OLED interface, temperature/humidity/pressure sensing, two low-current external motor-driver interfaces, isolated relay contacts, RGB and buzzer outputs, controlled I2C expansion, and future CAN/RS485 provisions. The 2.42-inch SSD1309 OLED and BME280 are reference implementations rather than permanent dependencies. The primary integration case is an external nominal 18 V lithium-ion tool-battery system; DeWalt 20V MAX is the initial reference implementation rather than a platform dependency.
